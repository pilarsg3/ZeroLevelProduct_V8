...
def create_primary_pump(
    ...
    # axis (y = 0) and extends in +Y through the barrel wall and the full flange,
    # leaving walls of barrel_wall_t on all four sides of the flange.
    ...
    _fb_len = barrel_radius + flange_width + 1.0
...
#     # ── Geometry: exactly the original build logic, unchanged ─────────────────
#     barrel_outer = cq.Workplane("XY").circle(barrel_radius).extrude(barrel_height)
...
#     # Built as an explicit box (not a workplane extrude) to avoid sign ambiguity
#     # in the XZ workplane normal direction.
#     _fb_w   = flange_depth  - 2 * barrel_wall_t   # bore width  (X)
#     _fb_len = barrel_radius + flange_width + 1.0   # bore length (Y), from axis to beyond flange
#     _fb_h   = flange_height - 2 * barrel_wall_t   # bore height (Z)
#     flange_bore_cutter = (
#         cq.Workplane("XY")
#         .box(_fb_w, _fb_len, _fb_h)
#         .translate((0.0, _fb_len / 2, flange_z_top - flange_height / 2))
...
#     result = barrel.union(nozzle_right).union(nozzle_left).union(flange).clean()
#     assy = cq.Assembly()
#     assy.add(result, name="pump")
#     return assy
